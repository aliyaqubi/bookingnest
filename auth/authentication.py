...
def get_token(request: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    customer = db.query(models.DbCustomer).filter(models.DbCustomer.username == request.username).first() 
    if not customer:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Invalid username")
    if not Hash.verify(customer.password, request.password):                                              
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Incorrect password")     
    
    access_token = oauth2.create_access_token(data={'sub' : customer.username})                          
...
def get_token(request: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    hotel = db.query(models.DbHotel).filter(models.DbHotel.username == request.username).first() 
    if not hotel:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Invalid username")
    if not Hash.verify(hotel.password, request.password):                                              
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Incorrect password")     
    
    access_token = oauth2.create_access_token(data={'sub' : hotel.username})                           
