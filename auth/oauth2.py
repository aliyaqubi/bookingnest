##>>> Whatis: Token generation code (Copy-Paste from Udemy)
...
##> Block 1: provide endpoint for token retrieval.
...
##> Block 2: Secret Key + create access token
...
##> Block 3: to retrieve the current admin that the token is attached to, & verify the token to make sure the admin is authenticated.
def get_current_admin(token: str = Depends(admin_oauth2_scheme), db: Session = Depends(get_db)):
  ...
  try:                                                               #> The try block is used to wrap code that may raise exceptions. 
    ...
    raise credentials_exception              # WTError is an exception from JWT library, which is raised by an issue with decoding the token.
...
##> Block 4: to retrieve the current customer that the token is attached to, & verify the token to make sure the customer is authenticated.
def get_current_customer(token: str = Depends(customer_oauth2_scheme),  db: Session = Depends(get_db)):
  ...
  customer = db_customer.get_customer_by_username(db, username)     
  if customer is None:
    raise credentials_exception
...
##> Block 5: to retrieve the current hotel that the token is attached to, & verify the token to make sure the hotel is authenticated.
